 ...
 * Feature: Core Logic AMM, Market Config, Cron Volatility, Staking Engine, & BOD Caching
 * Update: Added updateBoardOfDirectors() for Event-Driven CEO Sorting
 */
...
// ==========================================
global.marketConfig = {
    IVL: { initialPrice: 5000, circulatingSupply: 10000000, impactRatio: 0.10, volatility: 0.03, db: 'ivylink', apy: 50 },
    LBT: { initialPrice: 100000, circulatingSupply: 5000000, impactRatio: 0.15, volatility: 0.05, db: 'lilybit', apy: 150 },
    IRC: { initialPrice: 1000000, circulatingSupply: 1000000, impactRatio: 0.20, volatility: 0.05, db: 'iriscode', apy: 500 },
    LTN: { initialPrice: 10000000, circulatingSupply: 500000, impactRatio: 0.25, volatility: 0.06, db: 'lotusnet', apy: 1500 },
    RSX: { initialPrice: 100000000, circulatingSupply: 100000, impactRatio: 0.30, volatility: 0.07, db: 'rosex', apy: 5000 },
    TNX: { initialPrice: 1000000000, circulatingSupply: 50000, impactRatio: 0.40, volatility: 0.08, db: 'tulipnex', apy: 20000 }
};
...
if (!global.tradeEngine) {
    global.tradeEngine = {
        /**
         * 1. MARKET MAKER (Anti-Whale & Impact Boundary)
         */
        executeTransaction: function (ticker, amount, type = 'buy') {
            ...
            let rawImpact = amount / c.circulatingSupply;
            if (rawImpact < 0.01) rawImpact = 0.01; 

            let priceShiftPercent = c.impactRatio * rawImpact;
            let calculatedPrice = type === 'buy' 
                ? oldPrice * (1 + priceShiftPercent) 
                : oldPrice * (1 - priceShiftPercent);

            let maxChangeLimit = oldPrice * 0.50; 
            let highestAllowedPrice = oldPrice + maxChangeLimit;
            let lowestAllowedPrice = oldPrice - maxChangeLimit;
            
            let boundedPrice = Math.max(lowestAllowedPrice, Math.min(highestAllowedPrice, calculatedPrice));

            let absoluteMinPrice = Math.floor(c.initialPrice * 0.10);
            let absoluteMaxPrice = Math.floor(c.initialPrice * 50);

            let newPrice = Math.max(absoluteMinPrice, Math.min(absoluteMaxPrice, Math.round(boundedPrice)));
        ...
        /**
         * 2. EVENT-DRIVEN CACHING: Update Dewan Direksi (Top 3 TNX)
         * Dipanggil HANYA KETIKA ada transaksi koin TNX.
         */
        updateBoardOfDirectors: function () {
            let users = global.db.data.users;
            let market = global.db.data.settings.trading;
            
            if (!market) return null;

            // Sorting Top 3 (Heavy Task - O(N))
            ...
            // Simpan ke Cache
            market.bod = {
                ceo: tnxHolders[0] || [null, {}],
                kom: tnxHolders[1] || [null, {}],
                dir: tnxHolders[2] || [null, {}]
            };
        ...
        /**
         * 3. STAKING ENGINE: Kalkulasi Estimasi Profit
         */
        ...
        /**
         * 4. STAKING ENGINE: Panen Spesifik
         */
        ...
        /**
         * 5. STAKING ENGINE: Panen Keseluruhan Aset
         */
...
function startEngine() {
    if (!global.tradingCron) {
        console.log('🚀 [TulipNex] Cron Engine Started...');
        ...
        global.tradingCron = cron.schedule('* * * * *', async () => {
            try {
                ...
                // Pastikan Cache BOD ada saat bot pertama kali menyala
                if (!market.bod) {
                ...
                for (let t in global.marketConfig) {
                    ...
                    if (newPrice <= absoluteMinPrice) {
                        newPrice = absoluteMinPrice * (1 + c.volatility);
                    } else if (newPrice >= absoluteMaxPrice) {
                        newPrice = absoluteMaxPrice * (1 - c.volatility);
                    }
