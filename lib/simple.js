...
* Fixed: Asynchronous LID to PN Resolution to prevent database duplication.
*/
...
// FIX: Diubah menjadi async function agar bisa menunggu resolusi LID ke PN
conn.getJid = async (sender) => {
    ...
    // FIX: Menunggu proses pencarian nomor aslinya
    await resolveLidToPn(conn, jid).catch(() => {});
...
// FIX: Diubah menjadi mengecek via cache saja karena sync
                ...
                if (!message) continue
                    ...
                    if ((remoteJid && remoteJid !== 'status@broadcast') && quoted) {
                        ...
                        if (isGroup && !participant) participant = remoteJid
                            ...
                            // FIX: Menambahkan await
                            sender = await conn.getJid(message.key?.fromMe && conn.user.id || message.participant || message.key?.participant || chat || '')
                            if (sender !== chat) {
...
// FIX: smsg diubah menjadi async function
exports.smsg = async (conn, m, hasParent) => {
    ...
    if (m.key) {
        ...
        if (chatJid?.endsWith('@lid')) {
            chatJid = (await conn.getJid(chatJid)) || chatJid; // FIX: Menunggu konversi LID
        }
        ...
        // FIX: Menunggu konversi ID pengirim dari getJid
        let senderJid = m.key.fromMe 
            ? conn.user.id 
            : m.key.participantPn || m.key.senderPn ||
              (await conn.getJid(m.key.participant || m.key.remoteJid || m.key.senderLid || m.chat || '')) ||
    ...
    if (m.message) {
        ...
        // FIX: Menunggu konversi mentionedJid
        if (m.msg?.contextInfo?.mentionedJid) {
             m.mentionedJid = await Promise.all((m.msg.contextInfo.mentionedJid).map(async jid => (await conn.getJid(jid)) || jid));
        } else {
             m.mentionedJid = [];
        }
        ...
        if (m.quoted) {
            ...
            if (m.quoted) {  
                ...
                // FIX: Menunggu konversi ID pengirim yg di-reply
                m.quoted.sender = await conn.getJid(m.msg.contextInfo.participant)
                m.quoted.fromMe = m.quoted.sender === conn.user.jid
                m.quoted.text = m.quoted.text || m.quoted.caption || m.quoted.contentText || ''
                m.quoted.name = await conn.getName(m.quoted.sender)
                
                if (m.quoted.contextInfo?.mentionedJid) {
                     m.quoted.mentionedJid = await Promise.all((m.quoted.contextInfo.mentionedJid).map(async jid => (await conn.getJid(jid)) || jid));
                } else {
                ...
                m.getQuotedObj = m.getQuotedMessage = async () => {
                    ...
                    return await exports.smsg(conn, q)
                }
                ...
                m.quoted.copy = async () => await exports.smsg(conn, M.create(M.toObject(vM)))
