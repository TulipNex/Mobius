 ...
 */
async function getToken(instagramUrl) {
 ...
 */
async function getObfuscatedData(instagramUrl, token) {
...
/**
 * Fungsi utama untuk mengambil link download dari Instagram (Reel, Post, dll)
 * @param {string} instagramUrl - URL Instagram (reel, post, atau TV)
 * @returns {Promise<{videos: string[], images: string[]}>}
 */
async function scrapeInstagram(instagramUrl) {
  const token = await getToken(instagramUrl);
  const obfuscated = await getObfuscatedData(instagramUrl, token);
  const result = extractDownloadLinks(obfuscated);
  return result;
}

(async () => {
  const url = 'https://www.instagram.com/reel/DUt4CbgkilZ/?igsh=MTFvd2VxaTZ2OGticA==';
  try {
